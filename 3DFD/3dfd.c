...
int main (int argc, char *argv[])
{
	...
	float *p, *vx, *vy, *vz, *ro, *roz, *l2m, hcp, hro, fac;
	...
	ro = (float *)calloc(npx*npy*npz,sizeof(float));
...
*/
	...
	for (it=0; it<nt; it++) {
		...
		updateVelocitiesHalo(vx, vy, vz, p, ro, halo, npx, npy, npz);
		t1 = wallclock_time();
		thcomp += t1-t0;
		...
		updateVelocities(vx, vy, vz, p, ro, halo, npx, npy, npz);
		t2 = wallclock_time();
		tcomp += t2-t1;
