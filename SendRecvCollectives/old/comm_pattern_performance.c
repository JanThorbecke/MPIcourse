...
/*this is a naive implementation of a broadcast */
int broadcast(int *buffer,int count,int root){
...
/* This is a tree based algorithm, much faster and better scaling
  * than the naive one. */
    ...
    while(1){
        if(haveData){
            ...
            */
            ...
            depth=stat.MPI_TAG; //trick, use tag to transfer information
            haveData=1;
        }
        depth++;
    }
...
int main(int argc, char *argv[]){
    ...
    /* (a), let's try to do the broadcast operation. Let's also do
     * some performance comparison of the three algorithms that we
     * implemented */
    
    init_array(array,size);
    MPI_Barrier(MPI_COMM_WORLD);
    t1=MPI_Wtime();
    broadcast(array,chunk_size,root);
    t1=MPI_Wtime()-t1;
    MPI_Barrier(MPI_COMM_WORLD);
    printf("rank: %d broadcast time: %g s  size %g MB\n",
           rank,t1,(double)chunk_size*sizeof(int)/(1024*1024));

    
    init_array(array,size);
    MPI_Barrier(MPI_COMM_WORLD);
    t1=MPI_Wtime();
    broadcast_tree(array,chunk_size,root);
    t1=MPI_Wtime()-t1;
    MPI_Barrier(MPI_COMM_WORLD);
    printf("rank: %d broadcast_tree time: %g s size %g MB\n",
           rank,t1,(double)chunk_size*sizeof(int)/(1024*1024));
    
    init_array(array,size);
    MPI_Barrier(MPI_COMM_WORLD);
    t1=MPI_Wtime();
    MPI_Bcast(array,chunk_size,MPI_INT,root,MPI_COMM_WORLD);
    t1=MPI_Wtime()-t1;
    MPI_Barrier(MPI_COMM_WORLD);
    printf("rank: %d mpi_broadcast time: %g s size %g MB\n",
           rank,t1,(double)chunk_size*sizeof(int)/(1024*1024));
