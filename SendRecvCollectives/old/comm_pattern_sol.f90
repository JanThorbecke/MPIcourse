...
SUBROUTINE broadcast(buffer,count,root)
   ...
   IF (myid==root) THEN
     DO i=0,ntasks-1
       IF (i .NE. root) THEN
         CALL MPI_SEND(buffer,count,MPI_INTEGER,i,10,MPI_COMM_WORLD,ierror)
       END IF
     END DO
   ELSE
     CALL MPI_RECV(buffer,count,MPI_INTEGER,root,10,MPI_COMM_WORLD,MPI_STATUS_IGNORE,ierror)
   END IF
...
SUBROUTINE scatter(sendbuffer,count,recvbuffer,root)
  ...
  CALL MPI_COMM_SIZE(MPI_COMM_WORLD, ntasks, ierror)
   ...
   IF (myid==root) THEN
     DO i=0,ntasks-1
       IF (i .NE. root) THEN
         CALL MPI_SEND(sendbuffer(i*count+1:),count,MPI_INTEGER,i,11,MPI_COMM_WORLD,ierror)
       END IF
     END DO
     DO i=1,count
     ...
     END DO
   ELSE
     CALL MPI_RECV(recvbuffer,count,MPI_INTEGER,root,11,MPI_COMM_WORLD,MPI_STATUS_IGNORE,ierror)
   END IF
...
SUBROUTINE gather(sendbuffer,count,recvbuffer,root)
  ...
  IF (myid .NE. root) THEN
     CALL MPI_SEND(sendbuffer,count,MPI_INTEGER,root,12,MPI_COMM_WORLD,ierror)
   ELSE
     DO i=0,ntasks-1
       IF (i .NE. root) THEN
         CALL MPI_RECV(recvbuffer(i*count+1:),count,MPI_INTEGER,i,12,MPI_COMM_WORLD,MPI_STATUS_IGNORE,ierror)         
       END IF
     END DO
     DO i=1,count
       recvbuffer(root*count+i)=sendbuffer(i)
     END DO
   END IF
