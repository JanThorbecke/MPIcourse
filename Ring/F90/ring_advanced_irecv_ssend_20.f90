...
!     IF (left .EQ. -1) left = size-1
  ...
  DO i = 1, size
     ...
     CALL MPI_IRECV(rcv_buf, 1, MPI_INTEGER, left, to_right, MPI_COMM_WORLD, request, ierror)

     CALL MPI_SSEND(snd_buf, 1, MPI_INTEGER, right, to_right, MPI_COMM_WORLD, ierror)
